...
"""
def load_avg_costs(path_dir, ds_names, n_values, alpha_values=[1], error_values=[0]):
    avg_costs = {}
        ...
        if annotate_threshold is not None:
            for bar, h in zip(bars, heights):
                if h > annotate_threshold:
                    plt.text(bar.get_x() + bar.get_width()/2, annotate_threshold + 0.5, f"{h:.1f}",
...
def plot_zipf_parameter_sweep(avg_costs_per_alpha, alpha_values, ds_names, title, ylabel, annotate_threshold=None):
    ...
    for i, ds in enumerate(ds_names):
        heights = [avg_costs_per_alpha[alpha][ds][0] for alpha in alpha_values]  # pick n=2000 or first n
...
if __name__ == "__main__":
    ...
    # Standard Zipfian Test
    # alpha=1, error=0
    path_std = "results/StandardZipfianTest"
    avg_std = load_avg_costs(path_std, ds_names, n_values)
    plot_grouped_bar(avg_std[1][0], n_values, "Standard Zipfian Test (α=1)", "Average Search Cost", annotate_threshold=25)

    # Adversarial Zipfian Test
    # α=1, δ=0.9 
